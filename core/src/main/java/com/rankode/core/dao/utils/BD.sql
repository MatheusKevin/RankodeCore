...
CREATE TABLE IF NOT EXISTS `rankode`.`COLLABORATORS` (
  ...
  PRIMARY KEY (`DEVELOPERS_LOGIN`, `PROJECTS_ID`),
...
CREATE TABLE IF NOT EXISTS `rankode`.`HINTS` (
  ...
  `METRICS_INITIALS` VARCHAR(5) NOT NULL,
  PRIMARY KEY (`ID`),
  INDEX `fk_HINTS_METRICS2_idx` (`METRICS_INITIALS` ASC),
  CONSTRAINT `fk_HINTS_METRICS2`
    FOREIGN KEY (`METRICS_INITIALS`)
    REFERENCES `rankode`.`METRICS` (`INITIALS`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
-- Table `rankode`.`INDICATOR`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `rankode`.`INDICATOR` (
  `METRICS_INITIALS` VARCHAR(5) NOT NULL,
  `PROJECTS_ID` INT NOT NULL,
  `OWNER_LOGIN` VARCHAR(45) NOT NULL,
  `VISIBLE` TINYINT(1) NULL,
  PRIMARY KEY (`METRICS_INITIALS`, `PROJECTS_ID`, `OWNER_LOGIN`),
  INDEX `fk_METRICS_RULES_has_METRICS_METRICS1_idx` (`METRICS_INITIALS` ASC),
  INDEX `fk_INDICATOR_PROJECTS1_idx` (`PROJECTS_ID` ASC, `OWNER_LOGIN` ASC),
  CONSTRAINT `fk_METRICS_RULES_has_METRICS_METRICS1`
    FOREIGN KEY (`METRICS_INITIALS`)
    REFERENCES `rankode`.`METRICS` (`INITIALS`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_INDICATOR_PROJECTS1`
    FOREIGN KEY (`PROJECTS_ID` , `OWNER_LOGIN`)
    REFERENCES `rankode`.`PROJECTS` (`ID` , `OWNER_LOGIN`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `rankode`.`INTERVALS` (
  ...
  `MIN` INT NULL,
  `DESCRIPTION` VARCHAR(45) NULL,
  PRIMARY KEY (`METRICS_INITIALS`),
  CONSTRAINT `fk_INTERVALS_METRICS_RULES_has_METRICS1`
    FOREIGN KEY (`METRICS_INITIALS`)
    REFERENCES `rankode`.`INDICATOR` (`METRICS_INITIALS`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `rankode`.`INVITES` (
  ...
  INDEX `fk_INVITE_PROJECTS1_idx` (`PROJECTS_ID` ASC, `OWNER_LOGIN` ASC),
